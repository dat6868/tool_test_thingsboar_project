...
def get_sim_processes():
    ...
    for pid, cmdline in raw_list:
        match = re.search(r'(script_tool_simulator_device_.*?\.py_sim)', cmdline)
        if match:
...
def parse_device_sim_logs(active_scripts):
    ...
    for script in active_scripts:
        match = re.search(r'script_tool_simulator_device_(.*?)\.py_sim', script)
...
async def start_sim():
    try:
        ...
        script_name = f"script_tool_simulator_device_{suffix}.py_sim"
...
# === CONFIG_END ==="""
        ...
        if os.name == 'nt':
            subprocess.Popen([sys.executable, "-u", script_path], stdout=log_file, stderr=log_file, env=env)
        else:
            cmd = f"nohup {sys.executable} -u {script_path} > {log_path} 2>&1 &"
